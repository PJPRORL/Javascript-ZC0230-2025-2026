...
import type {Attack, Character, CharacterClass} from './types.js'
...
export class GameManager {
    ...
    private showCharacters(characters: Character[] = this.characters): void {
        console.log('\n--- Characters ---')
    ...
    private filterOnClass(filtered: Character[]) {
    ...
    private filterOnStat(stat: string): Character[] | null {
    ...
    private performAttack(attackPair: Attack): void {
    ...
    private attack(): void {
        ...
        const attackPair: Attack = [attacker, defender]
    ...
    private processChoice(choice: string): boolean {
        switch (choice) {
            case '1':
                this.filter()
                break
